...
function Header({ currentUser, backButton, setCurrentUser }) {
  ...
  return(
    <div className="header">
      {backButton ? (
        <>
        <span class="material-icons md-48" onClick={() => history.replace(backButton)}>
          arrow_back_ios
        </span>
        <span className="login">Hi {currentUser.first_name}!</span>
        
        </>
      ): (
        <>
        <Link to="/profile"><span className="material-icons md-48">face</span></Link>
        <span className="login">Hi {currentUser.first_name}!</span>
        </>
      )}
      <Link to="/">
      ...
      <span className="logo_text">Wander:Women </span>
      <Link to="/chat">
      <span className="material-icons md-48">try</span>
      </Link>
      <Link to="/explore" class="material-icons md-48">explore</Link>
